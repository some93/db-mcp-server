...
| 测试工程 | ⚠️ 当前用例已覆盖权限/事务/健康/缓存并发/HTTP MCP 协议入口/SSE session 管理/自定义路由映射/异步 `tools/call`/TDengine 写入关键路径，但 TDengine 更完整覆盖与更高层集成门禁仍不足 |
...
**现状**：此前报告中的“42 通过但关键路径零测试”已经过时。当前 `mvn test` 为 74 通过，以下问题已补覆盖，但仍有剩余缺口：
...
| 位置 | 问题 | 建议 |
|------|------|------|
| README.md:45 | 仅写"11 个 MCP 工具"易让人误以为不含 HTTP MCP 入口 | 已补充说明：11 个 MCP 工具 + 2 个 MCP HTTP 入口 + 2 个健康端点 = 15 个对外接口 |
| RequirementDocument.md 第 4 节 | 早期结论仍写"13 个接口" | 已修正为 15 个对外接口，并与 README 统一统计口径 |
...
| README.md / McpHttpSseDesign.md | `mcp.http.path`、`mcp.http.ssePath` 之前未真正驱动路由 | 已修复：当前已由 Spring MVC 占位符映射到实际路由，并补 `McpTransportControllerRouteTest` 验证 |
...
`test-report.md` 当前已更新为 74 通过，并补了权限异常 / 事务回滚 / 健康检查 / 缓存并发 / HTTP MCP 协议入口 / SSE session 生命周期 / 自定义路由映射 / async `tools/call` / TDengine 写入等关键路径测试；但“通过数”本身仍不能替代覆盖率或发布闸门。
...
测试工程视角的核心建议：**不要用测试数量作为质量证明，要用关键路径覆盖清单或覆盖率门禁**。当前 74 个测试已覆盖主要单元级风险、基础集成场景与 HTTP MCP 第二阶段异步入口，但离“有集成发布门禁”的成熟状态还有距离。
